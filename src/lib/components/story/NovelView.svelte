<script lang="ts">
  ...
  import { Book, Loader2, Send, X, Slash, ChevronDown, ChevronRight, Pencil, Trash2 } from 'lucide-svelte';
  ...
  const STORAGE_KEY_GENERATED = 'novel-generated-entries';
  ...
  // Track which entries were generated by instructions (separate from entries array)
  let generatedEntryIds = $state<Set<string>>(new Set());
  ...
  async function handleInstructionSubmit(cardId: string) {
    ...
    try {
      ...
      // Track that this entry was generated by an instruction
      generatedEntryIds = new Set([...generatedEntryIds, newEntry.id]);
  ...
  onMount(() => {
    ...
    // Load persisted generated entry IDs from localStorage
    const savedGenerated = localStorage.getItem(STORAGE_KEY_GENERATED);
    if (savedGenerated) {
      try {
        generatedEntryIds = new Set(JSON.parse(savedGenerated));
      } catch (e) {
        console.error('Failed to load generated entry IDs:', e);
      }
    }
  });
  ...
  // Persist generated entry IDs whenever they change
  $effect(() => {
    localStorage.setItem(STORAGE_KEY_GENERATED, JSON.stringify([...generatedEntryIds]));
  });
    ...
    <div class="mx-auto max-w-3xl space-y-3 sm:space-y-4">
      ...
      {:else}
        ...
        {#each story.entries as entry (entry.id)}
          <!-- Check if this entry was generated by an instruction - show BEFORE the entry -->
          {@const cardForEntry = instructionCards.find(c => c.generatedEntryId === entry.id)}
          {#if cardForEntry && generatedEntryIds.has(entry.id)}
            <!-- Instruction card shown BEFORE the generated entry -->
            <div class="group rounded-lg border border-accent-500/50 bg-accent-500/5 p-3 sm:p-4">
              <div class="flex items-start gap-3">
                <div class="flex flex-col items-center gap-1">
                  <button
                    onclick={() => toggleInstructionCollapse(cardForEntry.id)}
                    class="rounded p-1 text-accent-400 hover:bg-accent-500/20 hover:text-accent-300 cursor-pointer"
                  ...
                  <!-- Edit/Delete buttons below expand/collapse -->
                  <div class="flex flex-col gap-1">
                    <button
                      onclick={() => startEditInstruction(cardForEntry.id, cardForEntry.text)}
                      class="rounded p-1 text-accent-400 hover:bg-accent-500/20 hover:text-accent-300 flex items-center justify-center"
                    ...
                    <button
                      onclick={() => {
                        instructionCards = instructionCards.filter(c => c.id !== cardForEntry.id);
                        // Also remove from generated entry IDs if entry still exists
                        if (cardForEntry.generatedEntryId) {
                          generatedEntryIds = new Set([...generatedEntryIds].filter(id => id !== cardForEntry.generatedEntryId));
                        }
                      }}
                      class="rounded p-1 text-accent-400 hover:bg-accent-500/20 hover:text-accent-300 flex items-center justify-center"
              ...
              <div class="mt-3 flex items-center justify-center gap-4">
                <label class="flex items-center gap-2 text-sm text-surface-300">
                  ...
                  <input
                    ...
                    min="50"
                    max="10000"
                    class="w-20 px-2 py-1 bg-surface-900 border border-surface-600 rounded text-surface-200 text-sm no-spinners text-center"
                  />
                </label>
                <button
                  class="btn btn-primary px-4 py-1.5 text-sm flex items-center gap-1 cursor-pointer"
